...
module Log = (val Logs.src_log (Logs.Src.create ~doc:"Validates invariants using Z3" "valid"))
...
let check_verification_condition ctx env solver
      ...
      (gen_pred, gen_values) =
  set_solver_timeout ctx solver (Configuration.z3_default_timeout ());
  ...
  Log.debug (fun f -> f "checking that %s ==> %s" (Z3.Expr.to_string user_pre_pred) (Z3.Expr.to_string user_post_pred));
  ...
  | Some true ->
    ...
    set_solver_timeout ctx solver (Configuration.z3_challenging_timeout ());
...
let build_validator (data: VerificationCondition.verification_condition) =
  ...
  fun (gen_pred, gen_values) ->
    ...
    | Some true ->
      ...
      | `Valid ->
        Log.info (fun f -> f "\t         => PASSED@.");
        Log.info (fun f -> f "\ttest 03: checking preserved@.");
        let res =
        ...
        begin
          match res with
          | `Valid -> Log.info (fun f -> f "\t         => PASSED@.")
          | `InvalidPure -> Log.info (fun f -> f "\t         => FAILED (pure)@.");
          | `InvalidSpatial -> Log.info (fun f -> f "\t         => FAILED (spatial)@.");
        end;
        res
