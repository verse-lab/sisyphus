(* ultimate_cclosure.ml is derived from coq/kernel/cclosure.ml, extended with support for evaluating through opaque constants   *)
(************************************************************************)
    ...
    let v =
      try
        let body =
          ...
          | ConstKey cst ->
            if TransparentState.is_transparent_constant flags (fst cst) then Opaque_env.constant_value_in env cst
            else raise Not_found
        in
        Def (inject body)
      with
