(* ultimate_cbn.ml is derived from coq/kernel/cbn.ml, extended with support for evaluating through opaque constants   *)
...
struct
  ...
  let rec pr_member pr_c member =
    ...
    | Primitive (p,c,args,kargs,cst_l) ->
      ...
      ++ pr_comma () ++ pr pr_c args ++ str ")"
    | Cst {const=mem;curr;remains;params;cst_l; _} ->
  ...
  let will_expose_iota args =
    List.exists
      (function (Fix (_,_,l) | Case (_,l) |
                 Proj (_,l) | Cst {cst_l=l; _}) when Cst_stack.is_empty l -> true | _ -> false)
      args
         ...
         begin
          ...
          (* Looks for ReductionBehaviour *)
            ...
            | Some behavior ->
              ...
              | NeverUnfold -> fold ()
              | (UnfoldWhen { nargs = Some n; _ } |
                 UnfoldWhenNoMatch { nargs = Some n; _ } )
                when Stack.args_size stack < n ->
                fold ()
              | UnfoldWhenNoMatch { recargs; _ } -> (* maybe unfolds *)
           ...
           begin match behavior with
             ...
             | UnfoldWhenNoMatch { recargs; _ }-> (* maybe unfolds *)
               ...
               let volatile = match behavior with
                 | UnfoldWhen {nargs; _} -> Option.has_some nargs
