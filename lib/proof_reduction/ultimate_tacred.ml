(* ultimate_tacred.ml is derived from coq/tactics/tacred.ml, extended with support for evaluating through opaque constants   *)
(************************************************************************)
...
let value_of_evaluable_ref env evref u =
  ...
  | EvalConstRef con ->
    ...
    EConstr.of_constr (Opaque_env.constant_value_in env (con, u))
...
let eval_table = Summary.ref (Cmap.empty : frozen) ~name:"ultimateevaluation"
...
let invert_name labs l {binder_name=na0;_} env sigma ref na =
...
let match_eval_ref_value env sigma constr stack =
  ...
  | Const (sp, u) ->
    ...
    if is_evaluable env (EvalConstRef sp) then
      let u = EInstance.kind sigma u in
      Some (EConstr.of_constr (Opaque_env.constant_value_in env (sp, u)))
    else
      None
...
let rec red_elim_const env sigma ref u largs =
  ...
  let largs, unfold_anyway, unfold_nonelim, nocase =
    ...
    | Some (UnfoldWhen { nargs = Some n; _ } | UnfoldWhenNoMatch { nargs = Some n; _ })
      when nargs < n -> raise Redelimination
    | Some (UnfoldWhen { recargs = x::l; _ } | UnfoldWhenNoMatch { recargs = x::l; _ })
        ...
        (try
           ...
           if unf || is_evaluable env (EvalConstRef (Projection.constant p)) then
             ...
             (match unf, get (GlobRef.ConstRef (Projection.constant p)) with
              ...
              | false, Some (UnfoldWhen { recargs; _ } | UnfoldWhenNoMatch { recargs; _ })
