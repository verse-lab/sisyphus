(* ultimate_cbv.ml is derived from coq/pretyping/cbv.ml, extended with support for evaluating through opaque constants   *)
(************************************************************************)
...
let debug_cbv = CDebug.create ~name:"Ultimate_cbnCbv" ()
    ...
    let v =
      try
        let body = match ref with
          ...
          | ConstKey cst -> Opaque_env.constant_value_in info.env cst
        in
