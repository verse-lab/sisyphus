(* ultimate_reductionops.ml is derived from coq/kernel/reduction.ml, extended with support for evaluating through opaque constants   *)
(************************************************************************)
...
let constant_effect_table = Summary.ref ~name:"ultimate-reduction-side-effect" Cmap.empty
...
let inReductionEffect : Constant.t * string -> obj =
  declare_object @@ global_object_nodischarge "ULTIMATE-REDUCTION-EFFECT"
...
module ReductionBehaviour = struct
  ...
  let table =
    Summary.ref (GlobRef.Map.empty : t GlobRef.Map.t) ~name:"ultimatereductionbehaviour"
  ...
  let inRedBehaviour = declare_object {
                        (default_object "ULTIMATEREDUCTIONBEHAVIOUR") with
                        load_function = load;
...
let debug_RAKAM = CDebug.create ~name:"ULTIMATERAKAM" ()
  ...
  let rec whrec (x, stack) : state =
    ...
    | Const (c,u as const) ->
      ...
      if CClosure.RedFlags.red_set flags (CClosure.RedFlags.fCONST c) then
       ...
       match Opaque_env.constant_value_in env (c, u') with
       | body ->
         begin
...
let is_transparent e k =
  ...
  | Conv_oracle.Opaque -> true  (* ALWAYS TRANSPARENT *)
  | _ -> true
