(* ultimate_redexpr.ml is derived from coq/kernel/redexpr.ml, extended with support for evaluating through opaque constants   *)
(************************************************************************)
...
open Genredexpr
open Pattern
open Ultimate_reductionops
open Ultimate_tacred
open CClosure
open RedFlags
open Libobject
...
let warn_native_compute_disabled =
  CWarnings.create ~name:"ultimate-native-compute-disabled" ~category:"native-compiler"
  (fun () ->
...
let whd_cbn = Ultimate_cbn.whd_cbn

let simplIsCbn =
  Goptions.declare_bool_option_and_ref ~depr:false ~key:["UltimateSimplIsCbn"] ~value:false
...
let subst_strategy (subs,(local,obj)) =
  ...
  List.Smart.map
    (fun (k,ql as entry) ->
      let ql' = List.Smart.map (Ultimate_tacred.subst_evaluable_reference subs) ql in
...
let inStrategy : strategy_obj -> obj =
  declare_object {(default_object "ULTIMATESTRATEGY") with
...
let red_expr_tab = Summary.ref String.Map.empty ~name:"Declare Ultimate Reduction"
...
let warn_simpl_unfolding_modifiers =
  CWarnings.create ~name:"ultimate-simpl-unfolding-modifiers" ~category:"tactics"
         (fun () ->
...
let reduction_of_red_expr_val = function
  ...
  | Simpl (f,o) ->
     ...
     let am = if simplIsCbn () then Ultimate_cbn.norm_cbn f else simpl in
     (contextualize (if head_style then whd_am else am) am o,DEFAULTcast)
  | Cbv f -> (e_red (cbv_norm_flags f),DEFAULTcast)
  | Cbn f ->
     (e_red (Ultimate_cbn.norm_cbn f), DEFAULTcast)
...
let subst_red_expr subs =
  ...
  Redops.map_red_expr_gen
    (subst_mps subs)
    (Ultimate_tacred.subst_evaluable_reference subs)
    (Patternops.subst_pattern env sigma subs)

let inReduction : bool * string * red_expr -> obj =
  declare_object
    {(default_object "ULTIMATEREDUCTION") with
