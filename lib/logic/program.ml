...
type 'a stmt = [
  | `LetExp of Expr.param * 'a Expr.simple_shape * 'a stmt
  | `LetLambda of string * [ `Lambda of Expr.param list * 'a stmt ] * 'a stmt
  | `Match of 'a Expr.simple_shape * (string * string list * 'a stmt) list
...
type 'a lambda = [ `Lambda of Expr.param list * 'a stmt ]
...
let rec print_stmt print_expr : 'a stmt -> _ =
  ...
  | `LetExp (param, exp, rest) ->
    group (group (! "let" ^/^ Expr.print_param param ^/^ !"=") ^^ nest 2 (break 1 ^^ print_expr exp) ^/^ !"in") ^^ break 1 ^^
...
and print_lambda print_expr (`Lambda (params, body)) =
  let open PP in
  parens (string "fun" ^/^
   group (flow_map (break 1) Expr.print_param params) ^/^ string "->" ^/^
   nest 2 (print_stmt print_expr body))  
and print_case print_expr : (string * string list * 'a stmt) -> PP.document =
  let open PP in
  function
  |  (cons, [], body) ->
  ...
  | ("::", [h;t], body) ->
    group (nest 4 (group (string "|" ^/^ group (group (string h ^/^ string "::" ^/^ string t) ^/^ string "->"))) ^^
          nest 2 (break 1 ^^ group (print_stmt print_expr body)))

  | (cons, args, body) ->
    group (nest 4 (group (string "|" ^/^
           group (group (string cons ^/^ (parens (group (flow_map (string "," ^^ break 1) string args)))) ^/^ string "->"))) ^^
          nest 2 (break 1 ^^ group (print_stmt print_expr body)))
...
let print print_expr t =
  ...
  group (
    group (
      ! "let" ^/^ ! (t.name) ^/^ align (group (flow_map (break 1) (!) t.args)) ^/^ ! "=")  ^^
