...
val execution_trace: CompilationContext.t -> program -> ?st:CCRandom.state -> unit -> Parsetree.expression list * Sisyphus_tracing.trace
(** [execution_trace ctx p1] returns a function [unit -> args * trace] that
   can be called one or more times to generate deep execution traces
    of the program [p1] (the first element of the tuple are the asts of the [args] that generated the trace). *)
