...
module Bulma (H: HTML_SIG) = struct
  ...
  (** [navbar_menu (st, ed)] produces a Bulma navbar menu, with start elements [st] and end elements [ed] *)
  ...
  let level_item ?a_class:cls ?a elts =
    H.div ~a:(attrs_with_class a cls "level-item" []) elts
  ...
  let section ?a_class:cls ?a elts =
    H.section ~a:(attrs_with_class a cls "section" []) elts
