...
Lemma MList_is_MList_partial : forall A `{EA: Enc A} (l: list A) (r: loc) (lst: loc),
    r ~> MList_partial l lst ==> r ~> MList l.
Proof.
  intros. gen r. induction l.
  + intros r. simpl. rewrite !repr_eq. xsimpl*.
  + intros r. gen IHl. case l; [intros IHl | intros x l' IHl].
    ++ simpl. rewrite !repr_eq.  xpull. intros. xsimpl*. rewrite H. xsimpl*.
    ++ asserts Hun : (r ~> MList_partial (a :: x :: l') lst = \exists q, r ~~~> `{ hd' := Some a; tl' := Some q} \* q ~> MList_partial (x :: l') lst). {
         simpl. auto.
      }
      rewrite Hun; clear Hun. xpull. intros q. xchange IHl. rewrite (MList_eq r a _). xsimpl*.
Qed.
